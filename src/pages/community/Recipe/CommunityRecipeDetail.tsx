...
const CommunityRecipeDetail: React.FC = () => {
  ...
  const loggedInUser = { userNo: 1, username: '현재로그인유저' };
  ...
  useEffect(() => {
    ...
    const fetchRecipe = async () => {
      ...
      // --- 현재는 목(mock) 데이터로 테스트 ---
      // setRecipe({
      //   rcpNo: Number(recipeId),
      //   userInfo: { userNo: 2, username: '망곰eee', sik_bti: '육식 티라노', imageNo: 2, serverName: '/src/assets/sample/profile.png'},
      //   rcpName: '매콤팽이버섯',
      //   rcp_info: '매콤꾸덕 소스와 쫄깃한 팽이의 만남, 초간단 밥도둑 반찬 어때요?',
      //   created_at: '2025-07-15T08:00:00',
      //   imageNo :2,
      //   serverName : '/src/assets/sample/recipe.png',
      //   views: 50,
      //   stars : 3.5,
      //   delete_sataus: 'N',
      // });
    ...
    const fetchReviews = async (page: number) => {
      ...
      if (page === 1) { // 첫 페이지만 데이터가 있다고 가정
        ...
        //   reviewNo: i,
        //   userInfo: { userNo: i === 0 ? 1 : 10 + i, username: `리뷰어${i}`, sik_bti: '잡식 프로틴', imageNo: 1 , serverName : '/src/assets/sample/profile.png'},
        //   stars: 4.5 - (i * 0.5),
        //   content: `정말 맛있네요! ${i + 1}번째 후기입니다.`,
        //   serverName: i < 5 ? '/src/assets/sample/recipe.png' : undefined, // 5개만 포토리뷰
        ...
        // setPhotoReviews(mockReviewsData.filter(r => r.serverName));
  ...
  return (
    <>
      ...
      <div className={styles.container}>
        ...
        {/* 예시: 삭제/수정 버튼 */}
        {loggedInUser.userNo === recipe.userInfo.userNo && (
        ...
        <div className={styles.user_report}>
          <Link to={`/profile/${recipe.userInfo.userNo}`} className={styles.writer_profile}>
        ...
        <div className={styles.Reviews}>
          ...
          <div id={styles.Review}>
            ...
            <div className={styles.review_container}>
              {/* {reviews.map(review => (
                <React.Fragment key={review.reviewNo}>
                  <div id={styles.review}> */}
                    {/* ... 리뷰 내용, 프로필 등 JSX ... */}
                    {/* {loggedInUser.userNo === review.userInfo.userNo ? (
                      <div>
