...
const Reviews: React.FC<ReviewsProps> = ({ rcpNo }) => {
  ...
  useEffect(() => {
    fetchPhotoReviews();
    fetchReviews(false); // 정렬 변경 시에는 1페이지부터 다시 로드
  }, [rcpNo, sort]);
          ...
          <div className={styles.photo_container}>
            {photoReviews.slice(0, 4).map((review, index) => (
              <div key={review.reviewNo} className={styles.photo_box} onClick={() => handleOpenPhotoModal(index)}>
                {review.serverName && <img src={`/images/${review.serverName}`} alt="포토리뷰" />}
              </div>
            ))}
          ...
          <div className={styles.review_container}>
            {reviews.map(review => (
              <div key={review.reviewNo} className={styles.review_item}>
                ...
                {review.serverName && <img src={`/images/${review.serverName}`} alt="리뷰 사진" className={styles.review_img} />}
              </div>
            ))}
          </div>
          {page < totalPages - 1 && (
